# 🎯 CloudStock — DevOps & Developer Interview Q&A
...
### Q4: S3 bucket name uses `cloudstock-tf-state-{ACCOUNT_ID}`. Why not a simple static name?

**A:** S3 bucket names are **globally unique** across ALL AWS accounts worldwide. If someone else already has `cloudstock-tf-state`, your deployment fails. Using the AWS Account ID guarantees uniqueness per deployer.

```yaml
TF_STATE_BUCKET: cloudstock-tf-state-${{ steps.account.outputs.ACCOUNT_ID }}
```
...
### Q41: If an interviewer asks: "Can you show me the live deployed URL on AWS right now?", how do you explain it?

**A:**
> *"During our verification and testing phase, the entire microservices stack was deployed live on AWS ECS Fargate with an Application Load Balancer, Amazon RDS MySQL, and CloudWatch monitoring (all verified endpoints and metrics are documented with live screenshots and CloudWatch logs).*
> 
> *Because running continuous ECS Fargate tasks, an active Application Load Balancer, and RDS instances accrues ongoing cloud costs after the free tier period, I adhered to cloud financial management (FinOps) best practices by cleanly tearing down the live cloud resources using our automated teardown workflow.*
> 
> *The entire infrastructure is fully automated via CI/CD. It can be spun up from scratch into a fresh AWS environment in ~15 minutes with a single click on GitHub Actions. For immediate real-time walkthroughs, the container architecture runs locally via Docker Compose in under a minute."*
...
*Generated from the actual CloudStock codebase — every answer references real code and real decisions made in this project.*
