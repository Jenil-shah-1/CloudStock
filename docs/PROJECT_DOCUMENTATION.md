# 📖 CloudStock — Project Documentation
...
![CloudStock Live Demo](https://github.com/Jenil-shah-1/cloudstock/blob/feature/observability-stack/docs/demo.jpg)

CloudStock is a **production-grade e-commerce application** built with microservices architecture, deployed on AWS ECS Fargate with Stripe payment integration and real-time CloudWatch monitoring.
...
![Admin Panel](https://github.com/Jenil-shah-1/cloudstock/blob/feature/observability-stack/docs/admin_panel.jpg)
...
![Admin Products](https://github.com/Jenil-shah-1/cloudstock/blob/feature/observability-stack/docs/admin_prodcuts.png)
...
![Admin Orders](https://github.com/Jenil-shah-1/cloudstock/blob/feature/observability-stack/docs/admin_orders.png)

| Property | Value |
|----------|-------|
| Port | 4002 |
| Tech | Node.js, Express, mysql2/promise, Stripe SDK |
| Routes | /orders, /payments, /auth |
...
![Cart](https://github.com/Jenil-shah-1/cloudstock/blob/feature/observability-stack/docs/cart.png)
...
![Checkout](https://github.com/Jenil-shah-1/cloudstock/blob/feature/observability-stack/docs/Checkout.png)
...
![Order Status](https://github.com/Jenil-shah-1/cloudstock/blob/feature/observability-stack/docs/order_status.png)

![Receipt](https://github.com/Jenil-shah-1/cloudstock/blob/feature/observability-stack/docs/receipt.png)
...
┌─── Step 1: S3 Bucket ────────────────────────────────────────┐
│ • Get AWS Account ID                                          │
│ • Create cloudstock-tf-state-{ACCOUNT_ID} if not exists        │
│ • Enable versioning                                           │
└──────────────────────────────┬───────────────────────────────┘
                               ▼
...
│ • Output ALB URL to GitHub Summary                            │
└──────────────────────────────────────────────────────────────┘
```
...
```
1. Clean teardown workflow
2. All AWS resources removed
```

### Why Account-ID-Based Bucket?

S3 bucket names are globally unique. `cloudstock-tf-state` would conflict across accounts. Using `cloudstock-tf-state-{ACCOUNT_ID}` guarantees uniqueness per deployer.
...
| `ALB` | ALB, listener, target groups, path-based routing rules |
| `ECS` | ECS cluster, task definitions, services, ECR repos, Cloud Map namespace, Service Connect |
| `RDS` | RDS MySQL instance, DB subnet group, security group |
| `IAM` | ECS execution role + task role (ECR pull + CloudWatch logs + SSM) |
| `CloudWatch` | CloudWatch dashboard with 7 panels |
...
│
├── Cloud Map Namespace (cloudstock.local)
│   ├── product-service → port 4001
│   └── order-service → port 4002
│
├── ECR Repositories (5)
│   ├── cloudstock/product-service
│   ├── cloudstock/order-service
│   ├── cloudstock/frontend
│   ├── cloudstock/observability
│   └── cloudstock/db-init
│
└── CloudWatch
    ├── Log Group: /ecs/cloudstock (3-day retention)
    └── Dashboard: cloudstock-orders (7 panels)
```
...
Single log group `/ecs/cloudstock` with different stream prefixes (`product/`, `order/`, `frontend/`). Simplifies CloudWatch queries — one `SOURCE` for all dashboard widgets.

### 5. ECS Service Connect Over Static IPs

Internal services communicate via ECS Service Connect (via Cloud Map), providing DNS-based discovery that works across task replacements — no hardcoded IPs.
...
aws ecs describe-services --cluster cloudstock-cluster \
  --services product-service order-service frontend

# View recent logs
aws logs tail /ecs/cloudstock --since 1h --filter-pattern "ORDER_"
...
aws ecs execute-command --cluster cloudstock-cluster \
...
```bash
aws configure --profile cloudstock
```
...
```bash
aws configure set aws_session_token <your_session_token> --profile cloudstock
```

Activate the profile:
```bash
export AWS_PROFILE=cloudstock
```
...
aws ecs list-tasks --cluster cloudstock-cluster --service-name order-service

# Output:
# { "taskArns": ["arn:aws:ecs:us-east-1:123456:task/cloudstock-cluster/abc123def456"] }

# 2. Connect (use just the task ID part after the last /)
aws ecs execute-command \
  --cluster cloudstock-cluster \
  --task abc123def456 \
...
node -e "const m=require('mysql2/promise'); m.createConnection({host:process.env.DB_HOST,user:'admin',password:process.env.DB_PASSWORD,database:'cloudstock'}).then(c=>c.query('SELECT COUNT(*) as n FROM products').then(r=>{console.log(r[0]);c.end()}))"
...
| `execute command was not enabled` | Task started before `enable_execute_command` was added | Force new deployment: `aws ecs update-service --cluster cloudstock-cluster --service order-service --force-new-deployment` then wait 60s and retry with new task ID |
...
- Task role `cloudstock-ecs-task` with `AmazonSSMManagedInstanceCore` policy (`iam.tf`)
