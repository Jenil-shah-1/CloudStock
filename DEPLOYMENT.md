...
Go to: `https://github.com/<your-user>/cloudstock/settings/secrets/actions`
...
```bash
git clone https://github.com/Jenil-shah-1/cloudstock.git
cd cloudstock
...
aws s3 mb s3://cloudstock-tf-state-bucket --region us-east-1

# 3. Provision infrastructure (~15 min)
cd terraform
terraform init
terraform apply -var="db_password=CloudStock2024Strong"
...
for svc in product-service order-service frontend db-init; do
  docker build --platform linux/amd64 --provenance=false --no-cache \
    -t $AWS_ID.dkr.ecr.us-east-1.amazonaws.com/cloudstock/$svc:latest ./$svc
  docker push $AWS_ID.dkr.ecr.us-east-1.amazonaws.com/cloudstock/$svc:latest
done

# 5. Run DB migration
SUBNET=$(aws ec2 describe-subnets --filters Name=tag:Name,Values=cloudstock-private-1 --query 'Subnets[0].SubnetId' --output text)
SG=$(aws ec2 describe-security-groups --filters Name=group-name,Values=cloudstock-ecs-sg --query 'SecurityGroups[0].GroupId' --output text)
aws ecs run-task --cluster cloudstock-cluster --task-definition cloudstock-db-init \
  --launch-type FARGATE --network-configuration "awsvpcConfiguration={subnets=[$SUBNET],securityGroups=[$SG],assignPublicIp=DISABLED}"

# 6. Deploy services
aws ecs update-service --cluster cloudstock-cluster --service product-service --force-new-deployment
aws ecs update-service --cluster cloudstock-cluster --service order-service --force-new-deployment
aws ecs update-service --cluster cloudstock-cluster --service frontend --force-new-deployment

# 7. Wait & get URL
aws ecs wait services-stable --cluster cloudstock-cluster --services product-service order-service frontend
aws elbv2 describe-load-balancers --names cloudstock-alb --query 'LoadBalancers[0].DNSName' --output text
```
...
```
SOURCE '/ecs/cloudstock' | filter @message like /ORDER_BOOKED/ | parse @message /"amount":(?<amt>[\d.]+)/ | stats sum(amt) as Revenue by bin(1h)
```
